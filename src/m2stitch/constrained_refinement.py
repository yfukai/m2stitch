...
from .translation_computation import ncc, extract_overlap_subregion
...
def find_local_max_integer_constrained(func,init_x,limits,max_iter=100):
    ...
    for i in range(max_iter):
        around_x=[x + np.array(dxs) 
                  for dxs in itertools.product(*[[-1,0,1]]*dim)]
        around_x=[x for x in around_x 
...
def refine_translations(images,grid,r):
    for direction in ["north","west"]:
        for i2,g in tqdm(grid.iterrows(),total=len(grid)):
            ...
            init_values=[int(g[f"{direction}_x_second"]),
                         int(g[f"{direction}_y_second"])]
            limits=[[init_values[0]-r,init_values[0]+r],
                    [init_values[1]-r,init_values[1]+r]]
            values,ncc_value=find_local_max_integer_constrained(overlap_ncc,
                                                          init_values,
                                                          limits)
