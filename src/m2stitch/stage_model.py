...
def calc_liklihood(prob_uniform,mu,sigma,t):
    t2=-(t-mu)**2/(2*sigma**2)
    norm_liklihood = 1./(np.sqrt(2*np.pi)*sigma) * np.exp(t2)
    uniform_liklihood = 1/100.
    p = prob_uniform / 100.
    return p*uniform_liklihood + (1-p) * norm_liklihood

def compute_inv_liklihood(params,T):
    prob_uniform,mu,sigma = params
    return -np.sum([np.log(np.abs(calc_liklihood(prob_uniform,mu,sigma,t))) for t in T])

def compute_image_overlap(grid,direction,W,H,
                          max_stall_count=100,
                          prob_uniform_threshold=90):
    if direction=="north":
...
#    assert np.all(0 <= T < 100)
    ...
    for i in range(max_stall_count):
        ...
        model=minimize(compute_inv_liklihood,
                               init_guess,args=(T,),
                               bounds=[(0,100)]*3,
                               method="trust-constr")
...
def compute_repeatability(grid,overlap_n,overlap_w,W,H,pou):
    grid["north_valid1"]=filter_by_overlap_and_correlation(grid["north_y_first"],
                                                       grid["north_ncc_first"],
                                                       overlap_n,H,pou)
    grid["west_valid1"]=filter_by_overlap_and_correlation(grid["west_x_first"],
                                                           grid["west_ncc_first"],
                                                           overlap_w,W,pou)
    grid["north_valid2"]=filter_outliers(grid["north_y_first"],
                                         grid["north_valid1"])
    grid["west_valid2"] =filter_outliers(grid["west_x_first"],
                                         grid["west_valid1"])
...
def filter_by_repeatability(grid,r):
    for _,grp in grid.groupby("row"):
        ...
        if not any(isvalid):
            ...
            grid.loc[grp.index,"north_valid3"]=grp["north_x_first"].between(medx-r,medx+r)&\
    ...
    for _,grp in grid.groupby("col"):
        ...
        if not any(isvalid):
            ...
            grid.loc[grp.index,"west_valid3"]=grp["west_x_first"].between(medx-r,medx+r)&\
